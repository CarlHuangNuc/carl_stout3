...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        get_tokens = thinker_result["sequences"][:,input_ids.shape[1]:]
        
        if get_tokens.shape[1]< carl_custom_think_token:
            print("bring the end of ...rrrrrrrrrrrrrrrrrrrrr")
            print(thinker_result["sequences"])
        else:
            new_token = thinker_result["sequences"][:,-1:]
        
        full_tokens.append(get_tokens)
        total_tokens = 0
        while get_tokens.shape[1] == carl_custom_think_token and total_tokens < 2000:
            ...
            thinker_result = self.thinker.generate(input_ids = new_token,
                cache_position = cache_position,
                past_key_values=thinker_result["past_key_values"],
                **thinker_kwargs)


            if thinker_result["sequences"].shape[1]< carl_custom_think_token:
                print("bring the end of ...rrrrrrrrrrrrrrrrrrrrr")
                print(thinker_result["sequences"])
            else:
                new_token = thinker_result["sequences"][:,-1:]

            get_tokens = thinker_result["sequences"][:,1:]
            total_tokens += thinker_result["sequences"].shape[1] - 1
        ...
        exit()
        if generate_audio:
            print(type(full_hidden_states))
            print(len(full_hidden_states))

        
        if not generate_audio:
            return merged_tokens
        
        start_time_test3 = time.perf_counter()
        execution_time = start_time_test3 - start_time_test1
        print(f"text执行时间: {execution_time:.6f} 秒")
        ...
        sf.write("./merge_1.wav",wav.reshape(-1).detach().cpu().numpy(),samplerate=24000,)

        total_tokens = carl_custom_tokens 
        new_token = talker_result["sequences"][:,-1:]
        get_tokens  = carl_custom_tokens + 1
        while get_tokens == carl_custom_tokens + 1 and total_tokens < 2000:
            ...
            **{k: (v.to(input_ids.device) if torch.is_tensor(v) else v) for k, v in talker_kwargs.items()},
            ...
            if talker_result["sequences"].shape[1]< carl_custom_tokens + 1:
                print("bring the end of ...rrrrrrrrrrrrrrrrrrrrr")
                print(talker_result["sequences"])
                talker_generate_codes = talker_result["sequences"][:,  new_token.shape[1]: -1]
            else:
                talker_generate_codes = talker_result["sequences"][:,  new_token.shape[1]: ]
            ...
            full_wav.append(wav)
            get_tokens = talker_result["sequences"].shape[1] 
            new_token = talker_result["sequences"][:,-1:]
        ...
        start_time_test2 = time.perf_counter()
        execution_time = start_time_test2 - start_time_test1
